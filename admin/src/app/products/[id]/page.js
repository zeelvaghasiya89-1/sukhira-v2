...
export default function EditProductForm() {
  ...
  const addVariant = () => {
    setVariants([...variants, { name: '', sku: '', price: product.base_price, compare_at_price: null, stock: 10 }]);
  };
  ...
  if (loading) {
    return <div style={{ color: 'var(--admin-muted)' }}>Loading product details...</div>;
  }
  ...
  if (error && !product) {
    return (
      <div style={{ padding: '20px' }}>
        ...
        <Link href="/products" className="admin-btn admin-btn-secondary" style={{ marginTop: '20px' }}>Back to Products</Link>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '24px', fontSize: '14px', color: 'var(--admin-muted)' }}>
        <Link href="/products">Products</Link> / <span>Edit Product ({product?.name})</span>
      </div>
